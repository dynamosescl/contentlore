...
### 2026-04-30 — Audit + recovery from a 19h Pages-build outage

A scheduled audit caught that **every Pages deployment between 2026-04-29 17:06 and 2026-04-30 12:34 UTC failed silently** (~19 hours). All 13 Mod commits and the Phase-10 commits successfully built locally and pushed to GitHub, but Cloudflare Pages rejected the bundle at compile time, so production kept serving the last successful build (the pre-Mod #1 deployment). New Function paths (`/api/mod/*`, `/api/characters`, `/moderators/*`) fell through the SPA catch-all and returned the homepage HTML with a 200 status — masking the breakage from any naive smoke test that only checked HTTP codes.

**Root cause** (commit `8c1edc9`): a duplicate `const now` declaration inside `buildWeeklyDashboard` in `functions/creator-profile/[handle].js`. Line 361 declared `const now = Math.floor(Date.now() / 1000)` (seconds, for the existing 90-day session window). Line 431, added in Phase-10 Item 6, declared `const now = Date.now()` (ms, for the new schedule-pattern strip). esbuild rejected the file at compile time with `The symbol "now" has already been declared`, which broke the entire Functions bundle — not just this file. Fix: rename the second to `nowMs`. The smoke test that surfaced this caught it via `npx wrangler pages deployment list` showing 13 consecutive `Failure` rows; the manual `pages deploy` reproduces the esbuild error directly.

**Lesson:** trust HTTP code 200 less. SPA `_redirects` catch-alls turn missing Functions into a 200, so a route whose Function file exists on disk but didn't make it to production is indistinguishable from a working route at the curl-status-code level. Future smoke tests must assert `Content-Type: application/json` and a meaningful JSON shape on API endpoints, and assert page titles (not just status codes) on HTML routes. CI guard would help — `npx wrangler pages deploy --dry-run` would catch this before push.

**Audit fixes shipped:**

**Audit fix #1 — Multi-platform live state** (`4018975`). `/api/uk-rp-live` now polls every Twitch handle and every Kick handle exposed in the curated `socials` map, not just the primary. Each entry resolves its live state from whichever side is up, with a higher-viewer tiebreaker and the loser surfaced under `also_live`. Response gains `live_handle` so multi-view tiles can embed the actually-live channel even when it differs from `primary_platform`. Specifically unblocks dynamoses (primary=kick) when streaming on Twitch, plus 27 other dual-handle creators.

**Audit fix #2 — Network graph from session overlap** (`49937b3`). `creator_edges` has been empty since the regex-driven detector was written — UK GTA RP creators rarely narrate raids in stream titles. `/api/network` now derives `co-stream` edges from pairwise stream-session overlap (≥60 min in the last 30 days) on top of any explicit raid/host edges that do exist. Result: edge_count went from 0 → 135 on the first deploy. Hero/empty-state copy + EDGE_COLORS map updated to handle the new `co-stream` type.

**Audit fix #3 — Streaks privacy disclosure plain English** (`62af0b4`). "Anonymous UUID and store it in your browser's localStorage. It's sent to our API on every visit" → "tag your browser with an anonymous random ID so we can count your days". Same guarantee, no jargon.

**Audit fix #4 — Self-healing scheduler hooks** (scheduler version `a4b8dc39`). `maybeBackupDailyRecap` and `maybeGenerateClipOfDay` were both gated to a strict 14-min window per UTC day (06:00–06:14). When the underlying ContentLore API was returning HTML for 19 hours, both hooks ran but produced no rows because the JSON-parse fell through. Now: both hooks run on every cron tick from their target hour onwards. The endpoints themselves are idempotent (`ON CONFLICT(date) DO UPDATE`) and short-circuit without a Claude call when the row already exists, so calling repeatedly costs one D1 SELECT per tick. Daily-recap and clip-of-day should self-recover within the next 15 min cron tick after any transient API outage.

**Audit findings that turned out NOT to be bugs:**
- Mod dashboard not loading was a downstream symptom of the Pages-build failure — fixed once `8c1edc9` redeployed Functions.
- Creator approval (mod panel) was the same root cause: `saveCurated()` POSTed to `/api/admin/curated`, got HTML back, `.json()` failed.
- TikTok-style Feed Mode on `/gta-rp/clips/` was working code; it appeared broken because `state.clips` was empty (the underlying `/api/clips` was returning HTML for the same reason).
- Tech-jargon sweep: most matches were CSS class names (`.tick`, `.ticker`, `.tl-axis-tick`) and JS identifiers (`tick()`, `setInterval(tick, 1000)`). The `/gta-rp/fivem-enhanced/` page deliberately uses "scheduler" and "migration" — those refer to FiveM's resource scheduler and server-version migrations, which are the literal subject of the page. Only three real instances: streaks privacy copy (fixed), network footer "force-directed simulation, no external libraries" (fixed), network hero copy ("the type of interaction" → "how the two creators are connected", fixed).

**Confirmed all 23 hub/admin/dynamic pages return 200 with their real titles after the redeploy.** All 25 public APIs return JSON with `ok:true`. `/api/network` jumped from 0 to 135 edges. `/api/uk-rp-live` now exposes `live_handle` and `also_live` fields. Daily-recap and clip-of-day will auto-fill on the next 15-min scheduler tick.
